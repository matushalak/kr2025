...
"""
Variable mapping is as follows:
    var(r,c,v) = r*N*N + c*N + v
where r,c are in range (0...N-1) and v in (1...N).

To change the run to fit Non consecutive puzzles, look at the code between line 55-65 to un-comment the encoding for non-consecutive rule
"""
...
def to_cnf(input_path: str) -> Tuple[Iterable[Iterable[int]], int]:
    ...
    # comment this out to use for normal sudokus
    c5 = non_consecutive(all_vars)
    c6 = clues(no_val, mat)

    # comment + c5 out to use for normal sudokus
    constraints = c1 + c2 + c3 + c4 + c6 + c5
    if bug: print(f'Total {len(constraints)} clauses (constraints) and {nvars} propositional variables')
...
def clues(no_val:np.ndarray, mat:np.ndarray)->list[list[int]]:
    ...
    if bug: print(f'{len(constraint)} clue clauses')
    return constraint
...
def one_per_cell(all_vars:np.ndarray)->list[list[int]]:
    ...
    if bug: print(f'{len(constraint)} cell clauses')
    return constraint
...
def one_per_row(all_vars:np.ndarray)->list[list[int]]:
    ...
    if bug: print(f'{len(constraint)} row clauses')
    return constraint
...
def one_per_col(all_vars:np.ndarray)->list[list[int]]:
    ...
    if bug: print(f'{len(constraint)} column clauses')
    return constraint
    ...
    for box in boxes:
        for val_index in range(N):
            ...
            for comb in combinations(not_val_box, r=2):
                constraint.append(list(comb))
    if bug: print(f'{len(constraint)} box clauses')
    return constraint
...
def non_consecutive(all_vars:np.ndarray)->list[list[int]]:
    ...
    if bug: print(f'{len(constraint)} non-consecutive clauses')
    return constraint
